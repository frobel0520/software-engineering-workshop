  ...
  return (
    ...
    >
        ...
        {cicdLesson.sections.map((section, index) => {
          ...
          return (
            <article className="lesson-card" key={section.id}>
              ...
              <aside><small>記住這句</small><p>{step?.takeaway ?? "每個 pipeline 結果都要連回一個清楚的 gate boundary。"}</p></aside>
            </article>
          );
        })}
      </section>
      ...
      <section className="lab-cta">
        <div><p className="kicker">CI/CD LAB</p><h2>讓 workflow、failure 與 merge gate 都留下證據。</h2><p>{cicdLesson.objectives.length} 個學習目標 · deterministic fixture，不啟動真實 runner</p></div>
