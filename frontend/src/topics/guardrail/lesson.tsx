...
export function GuardrailLesson({ completed, onOpenLab, orientation }: { completed: boolean; onOpenLab: () => void; orientation: LessonOrientation }) {
  return (
    <TopicLessonShell
      ...
      description="從 Validator、Guard 到 OnFailAction，理解如何在 input、output 與 tool 三個掛載點建立可解釋、可重設的 guardrail pipeline。"
      ...
      <section className="lesson-list" aria-label="Guardrail 教材">
        {guardrailLesson.sections.map((section, index) => (
          <article className="lesson-card" key={section.id}>
            ...
            <aside><p>{guardrailLesson.objectives[index] ?? "讓每個失敗都留下可理解的理由。"}</p></aside>
          </article>
        ))}
      </section>

      <section className="lab-cta">
        <div><h2>把規則放進 pipeline。</h2></div>
