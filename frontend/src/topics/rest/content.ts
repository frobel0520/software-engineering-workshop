...
export const restLesson: LessonDefinition = {
  ...
  sections: [
    ...
    { id: "framework", title: "FastAPI 負責協調", body: "FastAPI 先比對 route，再解析並執行 dependencies，接著驗證 path、query 與 body；只有沒有驗證錯誤時才呼叫 path operation。" },
...
export const restTraceStages: readonly RestTraceStage[] = [
  ...
  { id: "cors", label: "02", actor: "CORS", summary: "可能先處理 OPTIONS preflight，再放行跨 origin request", fileId: "main.py" },
  { id: "routing", label: "03", actor: "Router", summary: "FastAPI 比對 method 與 path", fileId: "main.py" },
  { id: "dependency", label: "04", actor: "Depends", summary: "建立這次 request 使用的 Session", fileId: "database.py" },
  { id: "validation", label: "05", actor: "Pydantic", summary: "驗證 path、query 或 JSON body", fileId: "models.py" },
...
export const restScenarios: readonly RestScenario[] = [
  {
    ...
    responseBody: '{\n  "id": 3,\n  "name": "Keyboard",\n  "price": 1200\n}',
    status: "201 Created",
    sql: "INSERT INTO item (name, price) VALUES (?, ?); 參數：('Keyboard', 1200)",
  ...
  },
  {
    ...
    responseBody: '{\n  "id": 1,\n  "name": "Notebook",\n  "price": 180\n}',
    status: "200 OK",
    sql: "SELECT id, name, price FROM item WHERE id = ?; 參數：(1)",
    terminalStageId: "response",
    tone: "success",
  },
  {
    ...
    sql: "SELECT id, name, price FROM item WHERE id = ?; 參數：(99)",
    terminalStageId: "response",
    tone: "error",
  },
  {
...
export const restCodeFiles: readonly RestCodeFile[] = [
  ...
  },
  {
    ...
    lines: [
      ...
      { id: "db-4a", code: "connect_args = {\"check_same_thread\": False}", explanation: "允許 FastAPI 在不同 thread 間使用 SQLite；這不代表可以跨 request 共用同一個 Session。", timing: "建立 SQLite engine 前。", connection: "同步 dependency 與 path operation 可能由不同 thread 執行。", consequence: "省略它時，SQLite 可能因 thread 不同而拒絕使用 connection。", stages: ["database", "dependency"] },
      { id: "db-4", code: "engine = create_engine(\"sqlite:///workshop.db\", connect_args=connect_args)", explanation: "建立可連到 workshop.db 的 database engine，並套用 SQLite 的 thread 設定。", timing: "應用程式啟動並載入 module 時。", connection: "所有 Session 會透過這個 engine 發送 SQL。", consequence: "連線字串或 connect_args 錯誤會讓 database 操作失敗。", stages: ["database"] },
      ...
      { id: "db-4d", code: "    with Session(engine) as session:", explanation: "開啟短生命週期 Session，準備 workshop 的初始資料。", timing: "建表完成後、server 接受 request 前。", connection: "讓 GET 情境在全新 database 也有可預期的 rows。", consequence: "沒有 seed 時，全新 database 的 GET /items/1 只會得到 404。", stages: ["database"] },
      ...
      { id: "db-4f", code: "            session.add_all([Item(id=1, name=\"Notebook\", price=180), Item(id=2, name=\"Mouse\", price=650)])", explanation: "加入兩筆與 browser lab 相同的範例資料。", timing: "第一次啟動空 database 時。", connection: "這兩筆 row 支援 200 與 404 查詢對照。", consequence: "範例資料與 lab 漂移會讓教材展示和實際程式結果不同。", stages: ["database"] },
      { id: "db-4g", code: "            session.commit()", explanation: "提交 seed transaction，讓兩筆資料寫入 SQLite。", timing: "確認 database 尚未 seed 後。", connection: "後續每個 request 的 Session 都能查到它們。", consequence: "沒有 commit，Session 關閉後 seed 不會保存。", stages: ["database"] },
      ...
      { id: "db-7", code: "        yield session", explanation: "把 Session 注入 route，並暫停 dependency；request 結束後才離開 with。", timing: "path operation 執行前，到 response 完成後清理。", connection: "create_item 的 session 參數會收到這個物件。", consequence: "改成 return 仍可提供值，但失去 yield 後的清理生命週期。", stages: ["dependency"] },
