...
本文件把專案計畫轉成可驗證的系統需求，定義教材站服務誰、解決什麼問題、有哪些核心流程與規則。實作細節、component、route 命名、TypeScript 型別與 simulator API 留到專案級 SD。
...
| 編號 | 使用情境 | 結果 |
| --- | --- | --- |
| UC-01 | 學習者開啟課程地圖 | 看見 19 個主題、track 分組與 planned／ready 狀態 |
...
| ID | 需求 | 驗證方式 |
| --- | --- | --- |
| FR-01 | 系統必須從唯一課程清單呈現 19 個 topic 與 track | 檢查地圖與 `curriculum.json` 一致 |
...
- 課程定義屬於 repository 內容，以 `shared/curriculum.json` 為準。
...
1. 19 個 topic、兩個 track 以上的概念分組與 planned／ready 規則清楚。
